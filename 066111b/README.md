...
### Why are previews skipped for Dependabot PRs?
GitHub does not expose environment secrets to Dependabot-triggered workflow runs, by design. The preview workflow's `environment: previews` declaration scopes `PREVIEWS_DEPLOY_TOKEN` to that environment, so a Dependabot push cannot read the token, and the cross-repo `actions/checkout` of the previews repo fails with "Input required and not supplied: token". Rather than weaken the secret scoping for human contributors, the workflow has an `if: github.actor != 'dependabot[bot]'` guard at the `preview` job. The `pr-validate / build` job still runs and validates that the Jekyll source compiles after the action bump. Action-version bumps are almost always content-neutral, and the production deploy approval gate is the final safety net.

If you ever need to preview a Dependabot PR's effect on the rendered site, manually re-push the Dependabot branch from your own account (e.g. `git push origin dependabot/...:my-test-branch`), which runs the preview workflow under your actor identity with environment access.
...
- **Dependabot PRs**: weekly, merge after `pr-validate / build` passes. Note: preview deploys are intentionally skipped for Dependabot branches (GitHub does not expose environment secrets to Dependabot-triggered workflows). The build job still validates the bump; rely on the production deploy approval gate as the final safety net.
